...
import com.github.simulatan.meteornotificationsaddon.NotificationsAddon;
...
public class NotificationsHudElement extends HudElement {
	...
	public static final HudElementInfo<NotificationsHudElement> INFO = new HudElementInfo<>(
		NotificationsAddon.HUD_GROUP,
		"Notifications",
		"Displays various notifications on your HUD.",
		NotificationsHudElement::new);
			...
			if (mode.get() == Mode.SIMULATAN) {
				...
				for (int i = 0; i < notifications.size(); i++) {
					...
					final double x = baseX + (
						notification.getStartTime() == 0
							? 0
							: notificationTime <= animationDuration
								? (animationDuration - notificationTime) * getWidth() / animationDuration
								: notificationTime >= timeToDisplay - animationDuration
									? getWidth() - ((timeToDisplay - notificationTime) * getWidth() / animationDuration)
									: 0
					);
					...
					// Background
					if (radius > 0)
						DrawUtils.drawRoundedQuad(x, y, getWidth(), notificationHeight + progressBarHeight, radius, new Color(backgroundColor.get()));
					else
						DrawUtils.drawQuad(x, y, getWidth(), notificationHeight + progressBarHeight, new Color(backgroundColor.get()));

					// Progress bar
					double progress = (startTime + timeToDisplay - System.currentTimeMillis()) * getWidth() / timeToDisplay;
					if (radius > 0)
					...
					float titleX = titleAlignment == AlignmentX.Center ?
						(float) (x + getWidth() / 2 - DrawUtils.getWidth(notification.getTitle()) / 2) : titleAlignment == AlignmentX.Left ?
						(float) (x + titlePaddingX) :
						(float) (x + getWidth() - titlePaddingX - DrawUtils.getWidth(notification.getTitle()));
					...
					if (description != null && !description.isEmpty()) {
						// Description scale
						scale = Math.min(
							getWidth() * (1F - descriptionPaddingX / 100F)
						...
						float descriptionX = descriptionAlignment == AlignmentX.Center ?
							(float) (x + getWidth() / 2 - DrawUtils.getWidth(description) / 2) : descriptionAlignment == AlignmentX.Left ?
							(float) (x + descriptionPaddingX) :
							(float) (x + getWidth() - descriptionPaddingX - DrawUtils.getWidth(description));
